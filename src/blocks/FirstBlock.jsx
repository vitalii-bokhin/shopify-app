...
function FirstBlock(props) {
    ...
    const comparisonDate = useSelector((state) => state.datepicker.comparisonDate);
    ...
    if (comparisonDate.from && comparisonDate.to && data) {
        compareResultData = data.filter((item) => {
            const itemDate = new Date(item.date).setHours(0, 0, 0, 0);
            const dateFrom = new Date(comparisonDate.from).setHours(0, 0, 0, 0);
            const dateTo = new Date(comparisonDate.to).setHours(0, 0, 0, 0);
